    ...
    return (
        <nav className='w-full flex md:justify-center justify-between items-center p-4'>
            <div className='md:flex-[0.5] flex-initial justify-center items-center'>
                <img src="https://raw.githubusercontent.com/adrianhajdin/project_web3.0/main/client/images/logo.png" alt="logo" className='w-32 cursor-pointer' />
            </div>
            <ul className='text-white md:flex hidden list-none flex-row justify-between items-center flex-intial'>
                {["Market", "Exchange", "Tutorials", "Wallets"].map((el, index) => {
                    return <NavbarItem key={el + index} title={el} />
                })}
            ...
            <div className='flex relative'>
                ...
                {toggleMenu && (
                    <ul className='z-10 fixed top-0 -right-2 p-3 w-[70vw] h-screen shadow-2xl md:hidden list-none flex flex-col justify-start items-end rounded-md blue-glassmorphism text-white animate-slide-in'>
                        ...
                        {["Market", "Exchange", "Tutorials", "Wallets"].map((el, index) => {
                            return <NavbarItem key={el + index} title={el} classProps="my-2 text-lg" />
                        })}
                    </ul>
                )}
            </div>
